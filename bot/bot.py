...
class CompetitiveBot(BotAI):
    ...
    async def on_step(self, iteration: int):
        ...
        if (
            self.can_afford(UnitTypeId.SCV) and self.supply_workers + self.already_pending(UnitTypeId.SCV) < 65
            and cc.is_idle
        ):
            cc.train(UnitTypeId.SCV)
            
        # Build supply depots if we are low on supply, do not construct more than 2 at a time
        elif self.supply_left < 6:
        ...
        if self.structures(UnitTypeId.SUPPLYDEPOT):
            ...
            if self.structures(UnitTypeId.BARRACKS):
                if self.can_afford(UnitTypeId.MARINE) and self.units(UnitTypeId.MARINE).amount < 50:
                    self.train(UnitTypeId.MARINE)
                    self.build(UnitTypeId.BARRACKS, near=cc.position.towards(self.game_info.map_center, 8))

            # If we have at least one barracks that is compelted, build factory
            if self.structures(UnitTypeId.BARRACKS).ready:
                if self.structures(UnitTypeId.FACTORY).amount < 2 and not self.already_pending(UnitTypeId.FACTORY):
                    if self.can_afford(UnitTypeId.FACTORY):
                        position: Point2 = cc.position.towards_with_random_angle(self.game_info.map_center, 12)
        ...
        if self.structures(UnitTypeId.FACTORY):
            if self.can_afford(UnitTypeId.CYCLONE) and self.units(UnitTypeId.CYCLONE).amount < 50:
